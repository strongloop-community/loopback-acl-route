...
var Principal = require('loopback/lib/access-context').Principal;
var Role;
var ACL;
...
function acl(options) {
  ...
  return function(req, res, next) {
    ...
    if (!registry) {
      ...
      aclModel = options.aclModel || registry.getModelByType(loopback.ACL);
    }
      ...
      hasPrincipalFunc = function(req, principalType, principalId, cb) {
        ...
        if (principalType === Principal.ROLE) {
          ...
          if (!options.ignoreACLsFromDB) {
            ...
            aclModel.isMappedToRole(Principal.USER, userId, role,
              function(err, yes) {
              if (err) return cb(err);
              if (yes) return cb(null, true);
              aclModel.isMappedToRole(Principal.APP, appId, role,
                function(err, yes) {
                cb(err, yes);
              });
            });
